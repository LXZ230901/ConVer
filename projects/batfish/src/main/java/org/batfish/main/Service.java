...
public class Service {
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public JSONArray runTask(//调用batfish执行相应的任务
    ...
    try {
      ...
      System.out.println("---------------back-batfish-runtask-args:------------------");
      ...
      return new JSONArray(Driver.runBatfishThroughService(taskId, args));//执行这个任务?
    } catch (Exception e) {
      return new JSONArray(Arrays.asList(BfConsts.SVC_FAILURE_KEY, e.getMessage()));
    }
  }
