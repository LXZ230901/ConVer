...
class RibTree<R extends AbstractRoute> implements Serializable {
   ...
   * @return a {@link Set} of routes
   */
  public Set<R> getAllRoutes() {//更改了
    ImmutableSet.Builder<R> routes = ImmutableSet.builder();
    _root.collectAllRoutes(routes);
    return routes.build();
  }

  public List<R> getEveryRoutes() {//更改了
    List<R> routes = new ArrayList<>();
    _root.collectEveryRoutes(routes);
    return routes;
  }
  /**
  ...
  @Nullable
  RibDelta<R> mergeRouteStage(R route,boolean stage) {
    Prefix prefix = route.getNetwork();
    int prefixLength = prefix.getPrefixLength();
    long bits = prefix.getStartIp().asLong();
    return _root.mergeRouteStage(route, bits, prefixLength, 0, stage);
  }
