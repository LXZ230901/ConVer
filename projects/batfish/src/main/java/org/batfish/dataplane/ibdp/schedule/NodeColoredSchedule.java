...
public class NodeColoredSchedule extends IbdpSchedule {
  ...
  public NodeColoredSchedule(
      Map<String, Node> nodes, Coloring algorithm, IsisTopology isisTopology) {
    super(nodes);
    makeIsisGraph(nodes, isisTopology);
  ...
  private void makeIsisGraph(Map<String, Node> nodes, IsisTopology isisTopology) {
    /*
     * For the purposes of coloring, two nodes are adjacent if:
     * - They have established a BGP session
     */
...
//          session.getSrc().getOwner().getHostname(), session.getDst().getOwner().getHostname());
//    }
    for(IsisEdge edge:isisTopology.getEdges())
    {
      _graph.addEdge(
          edge.getNode1().getNode(),edge.getNode2().getNode()
      );
    }
    System.out.println("IsisGraph:");
