...
public abstract class AbstractRib<R extends AbstractRoute> implements GenericRib<R> {
  ...
  /**
   * Keep a Sorted Set of
   * alternative routes. Used to update the RIB if best routes are withdrawn
   */
  ...
  public final Set<R> getAllRoutes() {
    if (_allRoutes == null) {
      _allRoutes = ImmutableSet.copyOf(_tree.getAllRoutes());
    }
    return _allRoutes;
  }
  public final List<R> getEveryRoutes() {

    return _tree.getEveryRoutes();
  }
  ...
  @Nullable
  public RibDelta<R> mergeRouteGetDeltaStage(R route,boolean stage) {//更改
    RibDelta<R> delta = _tree.mergeRouteStage(route,stage);
    if (delta != null) {
      // A change to routes has been made
      _allRoutes = null;
      _logicalArrivalTime.put(route, _logicalClock);
