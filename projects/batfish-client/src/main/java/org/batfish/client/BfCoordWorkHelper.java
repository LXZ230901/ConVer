...
public class BfCoordWorkHelper {
  ...
  public boolean queueWork(WorkItem wItem) {
    ...
    try {
      ...
      WebTarget webTarget = getTarget(CoordConsts.SVC_RSC_QUEUE_WORK);//访问的是9997port,访问的是WorkMgrService
      System.out.println("-------------queuework-webtarget:------------");
      ...
      addTextMultiPart(multiPart, CoordConsts.SVC_KEY_API_KEY, _settings.getApiKey());//把这个对象送至后端排队处理
      System.out.println("BatfishObjectMapper.writeString(wItem):"+BatfishObjectMapper.writeString(wItem));
      JSONObject jObj = postData(webTarget, multiPart);
      if(jObj!=null) {
  ...
  public boolean uploadTestrig(
    ...
    try {
      ...
      System.out.println("-------------压缩成的zip配置文件----------");
