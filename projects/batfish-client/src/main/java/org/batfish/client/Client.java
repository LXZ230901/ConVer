...
public class Client extends AbstractClient implements IClient {
  ...
  public Client(Settings settings) {
    ...
    switch (_settings.getRunMode()) {//根据不同的参数选择不同的执行的方式，这里只是指不同的交互模式
      case batch:
      ...
      case genquestions:
        ...
        _logger =
            new BatfishLogger(_settings.getLogLevel(), false, _settings.getLogFile(), false, false);
        break;
      case interactive://交互模式的执行方式
        try {
          Terminal terminal = TerminalBuilder.builder().build();//创建新的终端
          //构建行读取器，读取命令行中的内容
          //创建输入和输出，输入输出都对应的是命令行
          _reader =
          ...
          historyPath.toFile().createNewFile();//创建文件的对象
          _reader.setVariable(LineReader.HISTORY_FILE, historyPath.toAbsolutePath().toString());
          System.out.println("--------------HISTORYPATH:---------------");
          System.out.println(historyPath.toAbsolutePath().toString());
          System.out.println("--------------END---------------");
          _reader
              .getTerminal()
  ...
  private boolean  execute(WorkItem wItem, @Nullable FileWriter outWriter) {
     System.out.println("work-id is "+ wItem.getId());
    ...
    boolean queueWorkResult = _workHelper.queueWork(wItem);//把要处理的配置文件送到队列当中去?
    ...
    if (!_backgroundExecution) {
      ...
      result = pollWorkAndGetAnswer(wItem, outWriter);//获取处理之后的结果状态的函数
  ...
  private boolean initTestrig(
    ...
    if (options.size() == 1) {
      if (options.get(0).equals("-autoanalyze")) {//ｏｐｔｉｏｎｓ只能对应一个ａｕｔｏａｎａｌｙｚｅ这一个参数
        autoAnalyze = true;
      } else {
    ...
    String testrigLocation = parameters.get(0);//配置文件所在的位置
    System.out.println("－－－－－－－－－－－－－－－－要分析的配置文件的位置：－－－－－－－－－－－－－－－－－－－－");
    System.out.println(testrigLocation);
    System.out.println("----------------------------End----------------------------");
    ...
    String testrigName =
        (parameters.size() > 1) ? parameters.get(1) : DEFAULT_TESTRIG_PREFIX + UUID.randomUUID();//分为两种情况：一种是指定名字(这种情况其就是路径后的第一个参数)，一种是不指定名字
    System.out.println("－－－－－－－－－－－－－－－－要分析的配置文件的名字：－－－－－－－－－－－－－－－－－－－－");
    System.out.println(testrigName);
    System.out.println("----------------------------End----------------------------");
    // initialize the container if it hasn't been init'd before
    if (!isSetContainer(false)) {
      System.out.println("－－－－－－－－－－－－－－－－SetContainer－－－－－－－－－－－－－－－－");//第一次运行需要设置容器，猜想是用来保存解析后的结果的
      _currContainerName = _workHelper.initContainer(null, DEFAULT_CONTAINER_PREFIX);//这只是个初始化的过程
      if (_currContainerName == null) {
        _logger.errorf("Could not init container\n");
        return false;
      }
    ...
    //每次分析都把他保存到一个临时文件夹当中
    System.out.println("－－－－－－－－－－－－－－－－_currContainerName：－－－－－－－－－－－－－－－－－－－－");
    System.out.println(_currContainerName);
    System.out.println("----------------------------End----------------------------");
    ...
    if (!autoAnalyze) {
      ...
      WorkItem wItemParse = WorkItemBuilder.getWorkItemParse(_currContainerName, testrigName);//创建要处理的对象?
      System.out.println("－－－－－－－－－－－－－－－－wItemParse：－－－－－－－－－－－－－－－－－－－－");
      System.out.println(wItemParse.toString());
      System.out.println("----------------------------End----------------------------");
      if (!execute(wItemParse, outWriter)) {//执行这个处理的对象?
        unsetTestrig(delta);
        return false;
      }
    }
  ...
  private boolean pollWork(UUID wItemId, @Nullable FileWriter outWriter) {//貌似只是为了获取当前的工作状态?
  ...
  private boolean pollWorkAndGetAnswer(WorkItem wItem, @Nullable FileWriter outWriter) {
    ...
    boolean pollResult = pollWork(wItem.getId(), outWriter);//貌似只是为了获取当前的工作状态?
    if (!pollResult) {
      return false;
    }
    // get the answer 结果的文件名,这只是一个最终一个状态的汇总,并不是处理结果保存的地方
    String ansFileName = wItem.getId() + BfConsts.SUFFIX_ANSWER_JSON_FILE;

    System.out.println("--------------结果的文件名:------------");
    ...
    System.out.println("--------------downloadedAnsFile:------------");
    System.out.println(downloadedAnsFile);
    System.out.println("-----------------END-----------------");
  ...
  private boolean processCommand(String command) {
    String line = command.trim();//去除掉字符串前面和后面的空格
    if (line.length() == 0 || line.startsWith("#")) {
      return true;
    }
    _logger.debugf("Doing command: %s\n", line);
    String[] words = line.split("\\s+");
    System.out.println("--------------Commands after split:---------------");
    ...
    System.out.println("");
    System.out.println("-----------End-------------");
    if (words.length > 0 && !validCommandUsage(words)) {//如果是无效的指令直接返回的是ｆａｌｓｅ
      return false;
    }
  ...
  boolean processCommand(String[] words, @Nullable FileWriter outWriter) {
    Command command;
    try {
      command = Command.fromName(words[0]);//获取当前命令的常量表示，具体的目标就是方便后续ｓｗｉｔｃｈ运算
    } catch (BatfishException e) {
    ...
    try (ActiveSpan span = GlobalTracer.get().buildSpan(command.commandName()).startActive()) {
      ...
      //返回指令的处理结果
      return processCommand(command, words, outWriter, options, parameters);//指令的名字＼指令的参数列表＼输出＼具体附加操作参数＼具体的处理参数
    } catch (Exception e) {
      e.printStackTrace();
      return false;
    }
  ...
  private boolean processCommand(
      ...
      List<String> parameters)
      throws Exception {
    System.out.println("－－－－－－－－－－指令的名字：－－－－－－－－－－");
    System.out.println(command);
    System.out.println("-----------------End------------------");
    switch (command) {//根据指令的名字选择指令的执行过程
  ...
  private void runInteractive() {//这才是真正的交互模型的运行的函数
    ...
    try {
      while (!_exit) {
        try {
          ...
          System.out.println("输入的参数："+rawLine);
          if (rawLine == null) {
            break;
          }
          processCommand(rawLine);//处理输入的参数
  ...
  private boolean uploadTestrig(String fileOrDir, String testrigName, boolean autoAnalyze) {//上传要分析的配置文件,首先变成zip文件后再上传,将配置文件上传到后端
    Path initialUploadTarget = Paths.get(fileOrDir);
    Path uploadTarget = initialUploadTarget;
    boolean createZip = Files.isDirectory(initialUploadTarget);//判断传入的对象是不是目录，增强鲁棒性
    if (createZip) {
      uploadTarget = CommonUtil.createTempFile("testrigOrEnv", "zip");//创建一个临时文件
      ZipUtility.zipFiles(initialUploadTarget.toAbsolutePath(), uploadTarget.toAbsolutePath());//将原文件存储到临时文件当中
    }
    try {
      boolean result =
