...
public class AsPath implements Serializable, Comparable<AsPath> {
  ...
  public void addAsPath(SortedSet<Integer> newAs)
  {
    this._asSets.add(newAs);
    return ;
  }
  ...
  public boolean subPreAsPath(String compareAsPath)
  {
//    String compareAsPath = asPath.getAsPathString();
    String thisAsPath = this.getAsPathString();
    thisAsPath=thisAsPath+" ";
    compareAsPath=compareAsPath+" ";
//    compareAsPath = compareAsPath.substring(1, compareAsPath.length()-2);
//    thisAsPath = thisAsPath.substring(1, thisAsPath.length()-2);
    if(thisAsPath.startsWith(compareAsPath))
    {
      return true;
    }
    return false;
