...
public class Main {
  ...
  private static void startWorkManagerService(
    ...
    if (_settings.getSslWorkDisable()) {
      ...
      URI workMgrUri =
          UriBuilder.fromUri("https://" + _settings.getWorkBindHost()).port(port).build();//在这个ip端口启动服务,对应服务源是WorkMgrService.class
  ...
  private static void initWorkManager() {
    ...
    startWorkManagerService(
        ...
        _settings.getServiceWorkPort());//启动这个类来为前端服务
  ...
  private static void mainRun() {
    try {
      ...
      initWorkManager();//初始化服务,启动batfishworkmgr服务
    } catch (Exception e) {
      System.err.println(
